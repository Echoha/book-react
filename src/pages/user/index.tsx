...
const COLUMNS = [
  ...
  },
  {
    ...
    width: 120,
    render: (text: string) => {
      return <Tag color={ text === STATUS.ON ? "green": "cyan"}>{`${STATUS_OPTIONS.find(obj => obj.value === text)?.label}`}</Tag>
    }
  },
  {
    title: '创建时间',
  ...
  const columns = [...COLUMNS,
  {
    title: '操作', key: "action", render: (_: any, row: any) => {
      return <>
        ...
        <Button type="link" danger={row.status === STATUS.ON ? true: false} onClick={()=> {
